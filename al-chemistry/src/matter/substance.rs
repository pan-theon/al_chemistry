...
impl Substance {
    pub fn from_elements(e: HashMap<String, (Element, u8)>) -> Result<Self, &'static str> {
        let checkers: Vec<fn(HashMap<String, (Element, u8)>) -> Result<Self, HashMap<String, (Element, u8)>>> =
            vec![Self::try_hydride, Self::try_oxide, Self::try_peroxide
                , Self::try_base, Self::try_salt, Self::try_acid];
    ...
    fn try_simple(mut e: HashMap<String, (Element, u8)>)
        -> Result<Self, HashMap<String, (Element, u8)>>
    {
        if e.len() != 1 { return Err(e); }
        ...
        Ok(Self { content, class: SubstanceClass::Simple, })
    }

    fn try_hydride(mut e: HashMap<String, (Element,u8)>)
        -> Result<Self, HashMap<String, (Element, u8)>>
    {
        if e.len() != 2 { return Err(e); }
        ...
        Ok(Self { content, class: SubstanceClass::Hydride, })
    }

    fn try_oxide(mut e: HashMap<String, (Element, u8)>)
        -> Result<Self, HashMap<String, (Element, u8)>>
    {
        if e.len() != 2 { return Err(e); }
        ...
        Ok(Self { content, class: SubstanceClass::Oxide, })
    }

    fn try_peroxide(mut e: HashMap<String, (Element, u8)>) 
        -> Result<Self, HashMap<String, (Element, u8)>>
    {
        if e.len() != 2 { return Err(e); }
        ...
        if el.1.0.group > 2 || el.1.0.group == el.1.0.period ||
            o2.1.1 != 2 ||
        ...
        Ok(Self { content, class: SubstanceClass::Peroxide, })
    }

    fn try_base(mut e: HashMap<String, (Element, u8)>)
        -> Result<Self, HashMap<String, (Element, u8)>>
    {
        // only hydroxides - inorganic bases
        if e.len() != 3 { return Err(e); }
        ...
        if el.0 == "N" && el.1.1 == 1 && o.1.1 == 1 && h.1.1 == 5 {
            ...
            content.insert("OH".to_string(), (SubstanceBlock::new(HashMap::from([o, h])), 1));

            return Ok(Self { content, class: SubstanceClass::Base, });
        }
    ...
    fn try_acid(mut e: HashMap<String, (Element, u8)>)
        -> Result<Self, HashMap<String, (Element, u8)>>
    {
        let mut h = match e.remove_entry("H") {
            Some(el) => el,
        ...
        for el in e.iter() {
            if el.1.0.group < 3 { e.insert(h.0, h.1); return Err(e); } 
            if el.1.0.group > 15 || el.1.0.electronegativity > 2.8 { is_oxidant = true; }

            residue = format!("{}{}{}", residue, el.0, el.1.1);
        }
        if !is_oxidant { e.insert(h.0, h.1); return Err(e); }

        let h_index = h.1.1;
        h.1.1 = 1;
        let mut content = HashMap::new();
        content.insert("H".to_string(), (SubstanceBlock::new(HashMap::from([h])), h_index));
        content.insert(residue, (SubstanceBlock::new(e), 1));

        Ok(Self { content, class: SubstanceClass::Acid, })
    }

    fn try_salt(mut e: HashMap<String, (Element, u8)>)
        -> Result<Self, HashMap<String, (Element, u8)>>
    {
        ...
        for el in e.into_iter() {
            ...
            } else if (el.1.0.period < 6 && el.1.0.group < 11 + el.1.0.period) && el.1.0.group < 16 {
                kind = &mut amphMe;
            } else {
                kind = &mut antiMe;
            }
            ...
            let mut i = 0;
            while i < kind.len() &&
                kind[i].1.0.electronegativity < el.1.0.electronegativity { i += 1; }
            kind.insert(i, el);
        }

        // there must be oxidant and Me
        if antiMe.len() == 0 || (Me.len() == 0 && amphMe.len() == 0) ||
            (antiMe[antiMe.len() - 1].1.0.group <= 15 &&
            antiMe[antiMe.len() - 1].1.0.electronegativity < 2.8 && o.is_none()) 
        {
            Me.append(&mut antiMe);
            Me.append(&mut amphMe);
            if let Some(h) = h { Me.push(h); }
            if let Some(o) = o { Me.push(o); }

            let mut err = HashMap::new();
            for m in Me { err.insert(m.0, m.1); }
            return Err(err);
        }
        if Me.len() == 0 {
            ...
            if let Some(mut h) = h {
                ...
                for i in 0..2 {
                    ...
                    for val in antiMe[antiMe.len() - 1].1.0.valencies.iter() {
                        ...
                        if guess_vacant_e >= 1 && guess_vacant_e <= *val as i16 &&
                ...
                if Me.len() == 0 && vacant_es[0] == 0 && vacant_es[1] == 0 {
                    ...
                    let mut err = HashMap::new();
                    for e in antiMe { err.insert(e.0, e.1); }
                    return Err(err);
                }
                ...
                if Me.len() == 0 && -vacant_es[1] == h.1.1 as i16 {
                    ...
                    o_oh.1.1 = 1;
                    h.1.1 = 1;
                    content.insert("OH".to_string(), (SubstanceBlock::new(HashMap::from([o_oh, h])), oh_index));
                }
        ...
        for mut m in Me {
            let m_index = m.1.1;
            m.1.1 = 1;
            content.insert(m.0.clone(), (SubstanceBlock::new(HashMap::from([m])), m_index));
        }
        ...
        Ok(Substance { content, class: SubstanceClass::Salt, })
    }
}
